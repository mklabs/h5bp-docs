...
helpers.server = function server(config) {
  ...
  connect.createServer()
    ...
    .use(connect.favicon(path.join(__dirname, 'templates', config.template, 'public/favicon.ico')))
...
helpers.copyAssets = function copyAssets(config, cb) {
  var src = config.assets ? config.assets : path.resolve(__dirname, 'templates', config.template, 'public'),
...
helpers.computeLayout = function computeLayout(config) {
  ...
  try {
    ...
    console.log('Unable to find ', path.join(basePath, config.layout), '. Fallback to ', path.join(__dirname, 'templates', config.template, 'index.html'));
    layout = fs.readFileSync(path.join(__dirname, 'templates', config.template, 'index.html'), 'utf8').toString();
  }
