...
task.registerBasicTask('emit', 'Reload funny things', function(data, name) {
  var socket = config(data.config);
  socket && socket.emit(data.event);
});
...
task.registerBasicTask('serve', 'Spawns up a local http server', function(data, name) {
  ...
  // path
  var dirname = path.resolve(data.path || name);
    ...
    fs.stat(filepath, function(e, stat) {
      ...
      fs.readFile(filepath, 'utf8', function(e, body) {
        ...
        body = body.replace(/<\/body>/, function(w) {
          console.log('replacing end body', w);
          return [
